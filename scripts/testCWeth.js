...
async function main() {
    ...
    const token = unn
    const cToken = ceth
    const amount = tokens('0.00001')

    console.log("[ STARTING CONDITIONS ]")
    for(let userIndex = 1; userIndex <= 3; userIndex++){
        console.log('user'+userIndex+' token balance', (await token.balanceOf(users[userIndex-1].address)).toString())
        console.log('user'+userIndex+' cToken balance', (await cToken.balanceOf(users[userIndex-1].address)).toString())
        console.log('user'+userIndex+' cToken underlying balance', (await cToken.callStatic.balanceOfUnderlying(users[userIndex-1].address)).toString())
        console.log('')
    }

    console.log("[ DEPOSIT EXECUTION ]")
    for(let userIndex = 1; userIndex <= 3; userIndex++){
        for(let x = 0; x < (4 - userIndex); x++){
            await cToken.connect(users[userIndex-1]).mint({value: amount})
            // await token.connect(users[userIndex-1]).approve(cToken.address, amount)
            // await cToken.connect(users[userIndex-1]).mint(amount)
            console.log('user'+userIndex+' -> deposited ' + amount)
            console.log('user'+userIndex+' cToken balance', (await cToken.balanceOf(users[userIndex-1].address)).toString())
            console.log('user'+userIndex+' cToken underlying balance', (await cToken.callStatic.balanceOfUnderlying(users[userIndex-1].address)).toString())
            console.log('')
        }
