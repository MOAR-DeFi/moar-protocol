...
import "./Interfaces/MDelegatorInterface.sol";
import "./MDelegationStorage.sol";
import "./MToken.sol";
import "./Moartroller.sol";
import "./AbstractInterestRateModel.sol";

/**
 * @title MOAR's CErc20Delegator Contract
 * @notice CTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author MOAR
 */
contract MErc20Delegator is MTokenInterface, MTokenStorage, MDelegatorInterface, MDelegationStorage {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, MTokenInterface cTokenCollateral) external  returns (uint) {
   ...
   /**
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setMoartroller(Moartroller newMoartroller) public returns (uint) {
        bytes memory data = delegateToImplementation(abi.encodeWithSignature("_setMoartroller(address)", newMoartroller));
        return abi.decode(data, (uint));
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setInterestRateModel(AbstractInterestRateModel newInterestRateModel) public returns (uint) {
