...
describe("Liquidity Simulation 1", () => {
  ...
  it("Test case 1 with protection", async () => {
    ...
    await app.meth._setMaxProtectionComposition(2500)
    await app.moartroller._setCollateralFactor(app.meth.address, tokens('0.5'))


    // Deposit ETH
    await app.meth.connect(app.user1).mint({value: tokens('2')})
    ...
    // Take a borrow
    await expectEvent(app.mdai.connect(app.user1).borrow(tokens('1000')), 'Borrow', {borrower: app.user1.address, borrowAmount: tokens('1000')})
    ...
    // -----
    // Set new price
    await app.oracle.setUnderlyingPrice(app.meth.address, tokens('1500'))
    ...
    await app.muunn.connect(app.user1).lockProtectionValue(2, tokens("0"))
    ...
    // Take another borrow
    await expectEvent(app.mdai.connect(app.user1).borrow(tokens('500')), 'Borrow', {borrower: app.user1.address, borrowAmount: tokens('500')})
  ...
  it("Test case 2 with protection", async () => {
    ...
    // Deposit ETH
    await app.meth.connect(app.user1).mint({value: tokens('20')})
    await app.moartroller.connect(app.user1).enterMarkets([app.mdai.address, app.meth.address])
