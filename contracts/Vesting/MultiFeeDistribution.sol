...
contract MultiFeeDistribution is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable  {
    ...
    function initialize(
        ...
        require(_stakingToken != address(0), "Address of StakingToken can`t be ZERO");
    ...
    function withdrawUnlockedStake() public nonReentrant updateReward(msg.sender) returns (uint256) {
        ...
        for (uint i = 0; i < length; i++) {
            ...
            if (userEarnings[msg.sender][i].unlockTime < block.timestamp) {
                earned = earned.add(earnedAmount);
                delete userEarnings[msg.sender][i];
            }
        }
        uint256 toWithdraw = unlocked.add(earned);

        totalSupply = totalSupply.sub(toWithdraw);
        bal.total = bal.total.sub(toWithdraw);
        bal.earned = bal.earned.sub(earned);
        bal.unlocked = 0;

        uint256 totalWithdraws = toWithdraw;

        if(userLocks[msg.sender].length > 0){
            totalWithdraws = totalWithdraws.add(withdrawExpiredLocks());
        }
    ...
    function earlyExitVests() external updateReward(msg.sender) returns (uint256){
        uint256 amount = 0;
        ...
        if (bal.earned > 0) {
            ...
            penaltyAmount = amount.mul(penalty).div(100);
            toWithdraw = amount.sub(penaltyAmount);
    ...
    function withdrawExpiredLocks() public returns (uint256){
